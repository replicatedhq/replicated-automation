...
kubectl exec $REPLICATED_POD_ID -c replicated -- replicatedctl app stop --force --attach

# Prevents issues on shutdown due to fs-snapshotter deployment
...
echo "Node Shutdown"
# Equivalent AWS command (for each node + LB): aws ec2 stop-instances --instance-ids i-07c817fd03dba9e0f && aws ec2 wait instance-stopped --instance-ids i-07c817fd03dba9e0f
